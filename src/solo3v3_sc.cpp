...
bool NpcSolo3v3::OnGossipHello(Player* player, Creature* creature)
{
    ...
    if (player->InBattlegroundQueueForBattlegroundQueueType((BattlegroundQueueTypeId)BATTLEGROUND_QUEUE_3v3_SOLO))
       AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "  |TInterface/ICONS/Achievement_Arena_2v2_7:26|t  Leave Solo 3v3 Queue\n", GOSSIP_SENDER_MAIN, 3, "Are you sure you want to remove the\n 3v3 Solo Queue?", 0, false);
    ...
    if (!player->GetArenaTeamId(ArenaTeam::GetSlotByType(ARENA_TEAM_SOLO_3v3)))
    {
        ...
        if (player->IsPvP())
            cost = 0;
        AddGossipItemFor(player, GOSSIP_ICON_TABARD, "  |TInterface/ICONS/Achievement_Arena_2v2_7:26|t  Create new Solo 3v3 Arena Team\n", GOSSIP_SENDER_MAIN, 1, "Create new Solo 3v3 Arena Team?", cost, false);
    }
    else
    {
        if (!player->InBattlegroundQueueForBattlegroundQueueType((BattlegroundQueueTypeId)BATTLEGROUND_QUEUE_3v3_SOLO))
        {
            ...
            AddGossipItemFor(player, GOSSIP_ICON_BATTLE, "  |TInterface/ICONS/Achievement_Arena_3v3_5:25:25:0:-12|t  Queue up for Solo 3v3 Arena\n", GOSSIP_SENDER_MAIN, 2);
            AddGossipItemFor(player, GOSSIP_ICON_BATTLE, " |TInterface/ICONS/Achievement_Arena_2v2_7:23|t Disband Solo 3v3 Arena team", GOSSIP_SENDER_MAIN, 5, "Are you sure?", 0, false);
        }

        AddGossipItemFor(player, GOSSIP_ICON_TRAINER, " |TInterface/ICONS/inv_inscription_weaponscroll01:23|t Show statistics", GOSSIP_SENDER_MAIN, 4);
    }
