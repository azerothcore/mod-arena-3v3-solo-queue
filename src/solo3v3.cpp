    ...
    for (int teamId = 0; teamId < 2; teamId++) // BG_QUEUE_PREMADE_ALLIANCE and BG_QUEUE_PREMADE_HORDE
    {
        ...
        for (BattlegroundQueue::GroupsQueueType::iterator itr = queue->m_QueuedGroups[bracket_id][index].begin(); itr != queue->m_QueuedGroups[bracket_id][index].end(); ++itr)
        {
            ...
            for (auto const& playerGuid : (*itr)->Players)
            {
                ...
                Solo3v3TalentCat playerTalentCat;
                if (filterTalents)
                    playerTalentCat = GetTalentCatForSolo3v3(plr);
                else
                    playerTalentCat = GetFirstAvailableSlot(soloTeam);
                ...
                bool allianceHasRoom = filterTalents
                    ? (getEffectiveSlot(TEAM_ALLIANCE) != MAX_TALENT_CAT)
                    : (queue->m_SelectionPools[TEAM_ALLIANCE].GetPlayerCount() != MinPlayersPerTeam);
                bool hordeHasRoom = filterTalents
                    ? (getEffectiveSlot(TEAM_HORDE) != MAX_TALENT_CAT)
                    : (queue->m_SelectionPools[TEAM_HORDE].GetPlayerCount() != MinPlayersPerTeam);

                // Prefer the team with no ignore conflict to avoid matching ignored players together
                ...
                for (int attempt = 0; attempt < 2 && !addedToTeam; ++attempt)
                {
                    ...
                    {
                        Solo3v3TalentCat effectiveSlot = filterTalents ? getEffectiveSlot(tryTeam) : playerTalentCat;
                        soloTeam[tryTeam][effectiveSlot] = true;
                        addedToTeam = true;
